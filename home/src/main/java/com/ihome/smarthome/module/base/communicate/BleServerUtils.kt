...
import com.ihome.smarthome.bleaction.BluetoothUtils.*
...
class BleServerUtils() {
   ...
   public companion object{
      public  fun newInstance() = BleServerUtils()
    }
...
private fun displayInfo(msg:String){
    ...
    EventBusUtils.sendLog("BleServerUtils",msg,LogEvent.LOG_DEBUG,true)
}
    /**
     * 蓝牙广播回调类
     */
    private inner class daqiAdvertiseCallback : AdvertiseCallback() {
        override fun onStartSuccess(settingsInEffect: AdvertiseSettings) {
            ...
            if (mTime != 0){
                ...
                EventBusUtils.sendLog("BleServerUtils","Ble广播结束",LogEvent.LOG_FAILED,true)
            }
    ...
    private inner class daqiBluetoothGattServerCallback : BluetoothGattServerCallback() {
        ...
        override fun onCharacteristicWriteRequest(device: BluetoothDevice, requestId: Int,
            ...
            Handler().postDelayed(Runnable {      //改变特征值
                characteristic.value = hexStringToBytes("abcdef")
                //回复客户端,让客户端读取该特征新赋予的值，获取由服务端发送的数据
                mBluetoothGattServer.notifyCharacteristicChanged(device,characteristic,false)  },100)
